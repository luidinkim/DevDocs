...
export const GameModeStartGame = dynamic(
  () => import('../../../components/UE5JumpMapComponents').then(mod => ({ default: mod.GameModeStartGame })),
  { ssr: false }
)

export const CoinCollection = dynamic(
  () => import('../../../components/UE5JumpMapComponents').then(mod => ({ default: mod.CoinCollection })),
  { ssr: false }
)

export const SaveGameData = dynamic(
  () => import('../../../components/UE5JumpMapComponents').then(mod => ({ default: mod.SaveGameData })),
  { ssr: false }
)
...
<Callout type="info" emoji="🎮">
  **인터랙티브 블루프린트 기능**
  ...
  - 🔗 **연결 생성**: 핀을 드래그하여 새로운 연결 만들기
  - 📐 **Smart Layout**: 우측 상단 버튼으로 자동 정렬
  - 🔍 **줌 인/아웃**: 마우스 휠로 확대/축소
  - 🎯 **노드 포커스**: 노드 클릭으로 선택 및 하이라이트
</Callout>
...
이 블루프린트는 플레이어가 코인을 수집했을 때 실행되며, 점수를 업데이트하고 업적 진행 상황을 확인합니다.
...
세이브 시스템은 플레이어의 진행 상황을 로컬에 저장합니다. 모든 데이터는 하나의 Save Object에 저장되어 효율적으로 관리됩니다.
