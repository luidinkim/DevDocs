...
export const TeleportPortalBlueprint = dynamic(
  () => import('../../../components/UE5BlueprintExamples_New').then(mod => ({ default: mod.TeleportPortalBlueprint })),
  { ssr: false }
)

export const PortalOverlapBlueprint = dynamic(
  () => import('../../../components/UE5BlueprintExamples_New').then(mod => ({ default: mod.PortalOverlapBlueprint })),
  { ssr: false }
)
...
<Callout type="info" emoji="🎮">
  **인터랙티브 블루프린트 기능**
  - 🖱️ **노드 드래그**: 모든 노드를 자유롭게 이동 및 재배치
  - 🔗 **연결 편집**: 핀을 드래그하여 새로운 연결 생성
  - 📐 **레이아웃 옵션**: 
    - Horizontal: 수평 자동 정렬
    - Vertical: 수직 자동 정렬
    - Radial: 방사형 배치
  ...
  - 📍 **미니맵**: 전체 그래프 개요 및 빠른 네비게이션
  - 🎨 **실행 흐름 시각화**: 흰색 실행 핀 강조 표시
</Callout>
