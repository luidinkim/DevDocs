...
export const JumpPadIntermediateBlueprint = dynamic(
  () => import('../../../components/UE5JumpPadComponents').then(mod => ({ default: mod.JumpPadIntermediateBlueprint })),
  { ssr: false }
)

export const HandleReactivationBlueprint = dynamic(
  () => import('../../../components/UE5JumpPadComponents').then(mod => ({ default: mod.HandleReactivationBlueprint })),
  { ssr: false }
)
...
<Callout type="info" emoji="🎮">
  **인터랙티브 블루프린트 기능**
  - 🖱️ **노드 드래그**: 모든 노드를 클릭하여 자유롭게 이동
  - 🔗 **연결 수정**: 핀을 드래그하여 연결 재구성
  - 📐 **레이아웃 모드**: 우측 상단 버튼으로 다양한 레이아웃 선택
    - Horizontal: 수평 정렬
    - Vertical: 수직 정렬
    - Radial: 방사형 정렬
  - 🔍 **줌 컨트롤**: 마우스 휠 또는 우측 상단 컨트롤
  - 🎯 **미니맵**: 전체 그래프 네비게이션
</Callout>
